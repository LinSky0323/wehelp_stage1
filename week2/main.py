...
def find_and_print(messages, current_station):
    #列出捷運線
    mrt=["Songshan","Nanjing Sanmin","Taipei Arena","Nanjing Fuxing","Songjiang Nanjin","Zhongshan","Beimen","Ximen","Xiaonanmen","Chiang Kai-Shek Memorial Hall","Guting","Taipower Building","Gongguan","Wanlong","Jingmei","Dapinglin","Qizhang","Xindian City Hall","Xindian"]
    #列出所有人在哪個站(用數字表示)
    local={}
...
def book(consultants, hour, duration, criteria):
    ...
    while True:
        ...
        for i in range(duration):
            if hour+i in AlreadyBook[consultants[th]["name"]]:
                check=False
                break
        #沒預約 把時間塞進AlreadyBook 印出可預約人
        if check==True:
            for i in range(duration):
                AlreadyBook[consultants[th]["name"]].append(hour+i)
            print(consultants[th]["name"])
            break
       # 全滿了
